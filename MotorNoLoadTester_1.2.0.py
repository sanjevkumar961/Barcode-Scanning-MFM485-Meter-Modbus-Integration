...
def getSheetLastRow(serial_name):
...
def sort_excel_sheets():
    """
    Reads an Excel file from the specified directory, sorts each sheet by 'Date' and 'S.No',
    and writes the sorted data back to the file while excluding specified sheets.
...
def verify_fitter(input_text, book, shift=3):
    try:
        ...
        display(f"File '{book}' not found.")
        return False
    except KeyError:
        display(f"Sheet '{"Users"}' not found in the workbook.")
        return False
    except Exception as e:
        display(f"An error occurred: {e}")
        return False
...
if not os.path.exists(os.path.join(excel_dir, excel_name)):
    ...
    root = tk.Tk()
    root.withdraw()
    fileCreate = messagebox.askyesno(
        title="CreateFile",
...
# Test strings
# test_strings = ["F4R-24/0001", "X4.2Y-25/0005", "F1.5R-24/0001", "X4.2Y-25/0001", "A5Z-245/0001", "X4.2Y-25/0003", "B1.5A-12/0001", "C10R-99/0001", "X4.2Y-25/0008"] # removee
...
try:
    ...
    while True:
        ...
        while True:
            ...
            if event.event_type == keyboard.KEY_DOWN:
                ...
                if is_valid_key(event.name):
                    SrNoUSER_INP += event.name
        if FitterName == None:
            # SrNoUSER_INP = "VpduwVdqmhy" # removee
            print(SrNoUSER_INP)
            FitterName = verify_fitter(SrNoUSER_INP, book)
            if FitterName:
                display(f"Fitter Name: {FitterName}")
            else:
                FitterName = None
                SrNoUSER_INP = ""  # Reset the old Value
                display(f"User Not Found, Please contact Admin.")
            print()
            continue
        SrNoUSER_INP = SrNoUSER_INP.upper()
        ...
        connect_verify()
        serial_name = (
            SrNoUSER_INP.split("-")[0] if "-" in SrNoUSER_INP else SrNoUSER_INP
        )
        ...
        if result is not None:
            ...
            display(
                f"The Amp value for model '{SrNoUSER_INP}' are MAX {MaxAmpValue} MIN {MinAmpValue}.\n",
                "white",
                True,
            )
        else:
            ...
            if fileCreate:
                ...
                display(
                    f"Model {serial_name} not found:- Test Cancelled: {SrNoUSER_INP}\n",
                    "yellow",
                    True,
                )
                continue
        ...
        for param, register in registers.items():
            ...
            if value is not None:
                ...
                # print(f"{param}: {value}")
                if param == "Amps":
                    sheet[number_to_column(excel_column) + str(excel_row + 1)] = (
                        CapacitorValue
                    )
                    excel_column += 1
                    AmpValue = param
                    if float(value) >= MinAmpValue and float(value) <= MaxAmpValue:
                        ...
                        if duplicate_check[0]:
                            cellHighlighter(
                                number_to_column(excel_column) + str(excel_row + 1)
                            )
                        excel_column += 1
                    else:
